...
export class PackGeneratorService {
  ...
  async generateZipBundle(artifacts: any[]): Promise<Readable> {
    ...
    // CRITICAL FIX: Download REAL files from S3 and stream into ZIP
    // This streams files directly from S3 without buffering in memory
    const downloadPromises = artifacts.map(async (artifact) => {
      if (artifact.binary?.s3Key) {
        try {
          // Get readable stream from S3
          const fileStream = await this.s3.getArtifactStream(artifact.binary.s3Key);

          // Stream the file into the ZIP archive
          archive.append(fileStream, {
            name: `artifacts/${artifact.binary.fileName}`,
          });
        } catch (error) {
          console.error(`Failed to download artifact ${artifact.id}:`, error);
          // Add error placeholder instead of failing the whole pack
          archive.append(
            `ERROR: Failed to download artifact\n` +
            `Name: ${artifact.name}\n` +
            `File: ${artifact.binary.fileName}\n` +
            `Error: ${error.message}\n`,
            { name: `artifacts/_ERROR_${artifact.binary.fileName}.txt` },
          );
        }
      }
    });

    // Wait for all files to be queued before finalizing
    await Promise.all(downloadPromises);

    // Finalize the archive (signal that all entries have been added)
    archive.finalize();
